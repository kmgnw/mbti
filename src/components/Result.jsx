...
function Result({score, resetState}){
    ...
    return(
        <div className='content' id='result'>
            <img id='img' className='rounded-circle' src={result.img}></img>
            <p id='qu'>나와 닮은 짱구 캐릭터는?</p>
            <p id='mbti'>{result.mbti}</p>
            <p id='explain' className='card card-body border-warning'>{result.explain}</p>
            <div className='result-btn'>
                <button className='btn btn-warning' onClick={handleRestart}>다시하기</button>
                <button className='btn btn-warning mt-4' onClick={handleCopyLink}>링크복사</button>
            </div>
        </div>
    )
}
